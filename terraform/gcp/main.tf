...
resource "google_compute_network" "gke_vpc_network" {
  name                    = "gke-qad-${random_id.unique.hex}"
  project                 = var.project_id
...
resource "google_compute_subnetwork" "gke_subnetwork" {
  ...
  secondary_ip_range {
    range_name    = "qad-secondary-pod"
    ip_cidr_range = "10.234.128.0/19"
  }

  secondary_ip_range {
    range_name    = "qad-secondary-svc"
...
module "gke" {
  ...
  ip_range_pods              = "qad-secondary-pod"
  ip_range_services          = "qad-secondary-svc"
  ...
  node_pools = [
    {
      ...
      auto_repair        = true
      auto_upgrade       = true
      preemptible        = var.preemptable_gke_nodes
  ...
  node_pools_metadata = {
    ...
    qad-node-pool = {
      node-pool-metadata-custom-value = "qad-applications"
    }
  }
